...
export default function ReadViewer() {
  ...
  return (
    <div className="flex flex-col h-screen bg-white text-slate-900 font-sans overflow-hidden">
      {/* Top Bar */}
      <header className="h-14 bg-slate-100 border-b border-slate-300 flex items-center justify-between px-4 z-20 shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-3.5 py-1.5 bg-red-700 hover:bg-red-800 text-white text-xs font-bold transition-all flex items-center gap-2 shadow-sm cursor-pointer disabled:opacity-50"
            title="Export A4 pages directly to PDF document"
          >
            <span>{isExportingPdf ? '⏳' : '📥'}</span>
            <span>{isExportingPdf ? 'Generating PDF...' : 'Export PDF Document'}</span>
          </button>
          <a
            href="/viewer/print"
            target="_blank"
            rel="noopener noreferrer"
            className="px-3 py-1.5 bg-slate-200 hover:bg-slate-300 text-slate-800 text-xs font-medium border border-slate-300 transition-colors flex items-center gap-1.5"
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Left Sidebar */}
        <aside className={`${sidebarCollapsed ? 'w-0' : 'w-80'} transition-all duration-200 bg-slate-100/95 border-r border-slate-300 flex flex-col overflow-hidden shrink-0`}>
        ...
        <aside className="w-96 bg-slate-100 border-l border-slate-300 flex flex-col overflow-hidden shrink-0">
