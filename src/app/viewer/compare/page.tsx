...
export default function CompareMode() {
  ...
  return (
    <div style={{
      minHeight: "100vh", background: "#F8FAFC", color: "#fff",
      ...
      <div style={{ maxWidth: "1450px", margin: "0 auto" }}>
        ...
        <header className="fu" style={{ marginBottom: "32px" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "10px", padding: "6px 14px", borderRadius: 0, background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.3)", marginBottom: "16px" }}>
            <div style={{ width: "6px", height: "6px", borderRadius: 0, background: "#ef4444" }} />
            <span style={{ fontSize: "10px", fontWeight: 900, color: "#ef4444", letterSpacing: "0.2em", textTransform: "uppercase" }}>5-Instrument Legal Analysis</span>
          </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "center", gap: "16px", marginBottom: "32px", borderBottom: "1px solid rgba(255,255,255,0.08)", paddingBottom: "16px" }}>
          <div style={{ display: "flex", gap: "12px" }}>
            <button
              ...
              style={{
                display: "flex", alignItems: "center", gap: "8px", padding: "10px 20px", borderRadius: 0,
                background: activeTab === "matrix" ? "#ef4444" : "rgba(255,255,255,0.03)",
            ...
            <button
              ...
              style={{
                display: "flex", alignItems: "center", gap: "8px", padding: "10px 20px", borderRadius: 0,
                background: activeTab === "cards" ? "#ef4444" : "rgba(255,255,255,0.03)",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <label style={{ fontSize: "11px", color: "#475569", textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: 700 }}>Filter Domain:</label>
            <select
              value={selectedCategory}
              onChange={e => setSelectedCategory(e.target.value)}
              style={{ padding: "8px 14px", borderRadius: 0, background: "rgba(2,6,23,0.8)", border: "1px solid #CBD5E1", color: "#fff", fontSize: "12px", outline: "none" }}
            >
              {categories.map(c => <option key={c} value={c} style={{ background: "#F8FAFC" }}>{c}</option>)}
            </select>
          </div>
        </div>

        {activeTab === "matrix" && (
          <div className="fu" style={{ overflowX: "auto", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 0, }}>
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "1250px", fontSize: "11px" }}>
              <thead>
                <tr style={{ background: "rgba(2,6,23,0.9)", borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
                  <th style={{ width: "14%", color: "#475569", textTransform: "uppercase", letterSpacing: "0.1em" }}>Statutory Topic</th>
                  ...
                  <th style={{ width: "18%", color: "#ef4444" }}>KBERC Bill 2026 (Enacted)</th>
                </tr>
              </thead>
              <tbody>
                {filteredData.map((row, idx) => (
                  <tr key={idx} style={{ background: idx % 2 === 0 ? "rgba(255,255,255,0.01)" : "transparent" }}>
                    <td>
                      ...
                      <span style={{ fontSize: "9px", fontFamily: "monospace", color: "#ef4444", background: "rgba(16,185,129,0.1)", padding: "2px 6px", borderRadius: 0, }}>{row.category}</span>
                    </td>
                    ...
                    <td style={{ background: "rgba(16,185,129,0.02)" }}>
                      ...
                      <div style={{ fontSize: "10px", color: "#ef4444", border: "1px solid rgba(16,185,129,0.3)", padding: "3px 6px", borderRadius: 0, background: "rgba(16,185,129,0.05)" }}>
                        💡 {row.proviso}
                      </div>
                    </td>
                  </tr>
        ...
        {activeTab === "cards" && (
          <div className="fu" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(360px, 1fr))", gap: "24px" }}>
            {filteredData.map((row, idx) => (
              <div key={idx} className="card-hover fu" style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 0, padding: "24px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "12px" }}>
                    <span style={{ fontSize: "9px", fontFamily: "monospace", color: "#ef4444", background: "rgba(16,185,129,0.1)", padding: "3px 8px", borderRadius: 0, }}>{row.category}</span>
                  </div>
                  <h3 style={{ fontSize: "16px", fontWeight: 800, color: "#fff", marginBottom: "16px" }}>{row.topic}</h3>

                  <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "16px" }}>
                    <div style={{ background: "rgba(239,68,68,0.04)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 0, padding: "8px 10px" }}>
                      <div style={{ fontSize: "9px", fontWeight: 900, color: "#EF4444", textTransform: "uppercase", marginBottom: "2px" }}>Cap 525 (1934 Legacy Act)</div>
                      <div style={{ fontSize: "11px", color: "#64748B" }}>{row.cap525}</div>
                    </div>
                    <div style={{ background: "rgba(245,158,11,0.04)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 0, padding: "8px 10px" }}>
                      <div style={{ fontSize: "9px", fontWeight: 900, color: "#F59E0B", textTransform: "uppercase", marginBottom: "2px" }}>Proposed Architects Bill</div>
                      <div style={{ fontSize: "11px", color: "#64748B" }}>{row.architectsBill}</div>
                    </div>
                    <div style={{ background: "rgba(236,72,153,0.04)", border: "1px solid rgba(236,72,153,0.2)", borderRadius: 0, padding: "8px 10px" }}>
                      ...
                      <div style={{ fontSize: "11px", color: "#64748B" }}>{row.qsBill}</div>
                    </div>
                    <div style={{ background: "rgba(96,165,250,0.04)", border: "1px solid rgba(96,165,250,0.2)", borderRadius: 0, padding: "8px 10px" }}>
                      ...
                      <div style={{ fontSize: "11px", color: "#64748B" }}>{row.aqsBill}</div>
                    </div>
                    <div style={{ background: "rgba(16,185,129,0.06)", border: "1px solid rgba(16,185,129,0.3)", borderRadius: 0, padding: "10px 12px" }}>
                      <div style={{ fontSize: "9px", fontWeight: 900, color: "#ef4444", textTransform: "uppercase", marginBottom: "2px" }}>KBERC Bill 2026 (Enacted Public-Safety Model)</div>
                      <div style={{ fontSize: "11px", color: "#fff", fontWeight: 600, lineHeight: 1.5 }}>{row.kberc2026}</div>
                    </div>
                  </div>

                  <div style={{ fontSize: "10px", color: "#ef4444", background: "rgba(16,185,129,0.08)", padding: "6px 10px", borderRadius: 0, }}>
